...
install_client() {
  ...
  [ -n "$workspace" ] || {
    printf 'WORKSPACE_DIR is missing from .env.\n' >&2
    exit 1
  }
  [ -n "$password" ] || {
    printf 'OPENCODE_SERVER_PASSWORD is missing from .env.\n' >&2
    exit 1
  }
...
prompt_secret 'GitHub personal access token' GITHUB_PERSONAL_ACCESS_TOKEN false
prompt_secret 'Postman API key' POSTMAN_API_KEY false
